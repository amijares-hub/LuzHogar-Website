      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        <AnimatePresence mode="popLayout">
          ...
          >
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-t from-titan-orange via-titan-orange/50 to-transparent" />
          </motion.div>
        </AnimatePresence>
      </div>
            ...
            return (
              ...
              >
                <img
                  src={item.imageSrc}
                  alt={item.title}
                  className="h-full w-full rounded-2xl object-cover pointer-events-none grayscale hover:grayscale-0 transition-all duration-700"
                />
        ...
        <div className="mx-auto mt-12 flex w-full max-w-4xl flex-col items-center justify-between gap-6 md:flex-row pointer-events-auto">
          <div className="flex flex-1 flex-col items-center text-center md:items-start md:text-left h-32 justify-center">
            <AnimatePresence mode="wait">
              ...
              >
                {activeItem.meta && (
                  <span className="text-xs font-black uppercase tracking-[0.3em] text-white underline decoration-titan-dark/20 underline-offset-4">
                    {activeItem.meta}
                  </span>
                )}
                <h2 className="text-3xl font-black tracking-tighter md:text-5xl text-white uppercase font-sans">
                  {activeItem.title}
                </h2>
                {activeItem.description && (
                  <p className="max-w-md text-white/80 text-xs uppercase tracking-widest leading-relaxed">
                    {activeItem.description}
                  </p>
                )}
              </motion.div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-1 rounded-full bg-white/20 p-1 ring-1 ring-white/10 backdrop-blur-md">
              <button
                onClick={handlePrev}
              ...
              <span className="min-w-[40px] text-center text-[10px] font-black text-neutral-500 uppercase tracking-widest">
                {activeIndex + 1} / {count}
              </span>
              <button
                onClick={handleNext}
            ...
            {activeItem.href && (
              <button
                onClick={() => onNavigate?.(activeItem.href!)}
                className="group flex items-center gap-3 rounded-full bg-titan-orange px-8 py-4 text-[10px] font-black uppercase tracking-[0.3em] text-white transition-all hover:scale-105 active:scale-95 shadow-xl"
              >
                Explorar
