  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <p className="mx-auto max-w-xs text-[10px] font-bold leading-relaxed text-gray-500 md:mx-0 uppercase tracking-widest">{mainText}</p>
          <motion.button 
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            onClick={onReadMoreClick}
            className="mt-6 inline-block text-[11px] font-black text-titan-dark uppercase tracking-[0.2em] italic border-b-2 border-titan-orange pb-1"
          >
            EXPLORAR MÁS
          </motion.button>
        </motion.div>
        ...
        {/* Center Carousel */}
        <div className="relative order-1 md:order-2 flex justify-center items-center h-full group">
            <motion.div
                initial={{ scale: 0.8, opacity: 0 }}
                animate={{ scale: 1, opacity: 1 }}
                transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1], delay: 0.2 }}
                className="absolute z-0 h-[280px] w-[280px] rounded-full bg-titan-orange/10 md:h-[380px] md:w-[380px] lg:h-[480px] lg:w-[480px]"
            ></motion.div>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <motion.img
                    ...
                    alt={imageAlt}
                    className="h-auto w-64 md:w-80 lg:w-96 object-contain drop-shadow-[0_20px_50px_rgba(0,0,0,0.1)] mb-4"
                    referrerPolicy="no-referrer"
                />
                <motion.span 
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="text-[9px] font-black uppercase tracking-[0.5em] text-titan-orange italic"
                >
            ...
            <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 flex justify-between px-4 z-20 opacity-0 group-hover:opacity-100 transition-opacity">
              <button 
                ...
                className="p-2 bg-white/80 backdrop-blur-sm rounded-full shadow-lg text-titan-dark hover:bg-titan-orange hover:text-white transition-all transform hover:-translate-x-1"
              >
                <ChevronLeft size={20} />
              </button>
              <button 
              ...
              {carouselItems.map((_, i) => (
                <button
                  ...
                  className={cn(
                    ...
                    currentIndex === i ? "w-8 bg-titan-orange" : "w-2 bg-gray-200"
                  )}
                />
              ))}
            </div>
        ...
        >
          <h1 className="text-6xl md:text-8xl lg:text-9xl font-black text-titan-dark tracking-tighter uppercase italic leading-none text-right">
            {overlayText.part1}
            <br />
            ...
            {showMap && (
              ...
              >
                <button 
                  ...
                  className="absolute top-2 right-2 p-1 bg-white/80 rounded-full text-titan-dark z-10"
                >
                  <X size={14} />
                </button>
